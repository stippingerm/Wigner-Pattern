...
if [ -z "$mfunction" ]
then
  mfunction=gpfa4syntheticSection
  mfunction=gpfa4runSection
fi
...
printf -v pattern "s/|animal|/$animal/;s/|mode|/$mode/;s/|in|/$in/;s/|out|/$out/;s/|zDim|/$zDim/;s/|bin_size|/$bin_size/;s/|fr|/$fr/;"
sed "$pattern" ../gpfa4runTemplate.txt > ../${mfunction}Settings.m
...
then
  ...
  echo MATLAB_PREFDIR=$PREFDIR matlab -nodisplay -r "$PRECOMMAND; try; $mfunction; catch ME; $ERRHANDLING; end; exit;"
fi
